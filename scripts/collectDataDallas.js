#!/usr/bin/env node
/**
 * Dallas DART Light Rail - Data Collector
 *
 * Polls Dallas DART GTFS-RT API every 90 seconds and saves vehicle positions to Supabase.
 * Speed is calculated from consecutive GPS readings.
 *
 * DART provides GTFS-RT feeds (as of April 2024):
 * https://www.dart.org/about/news-and-events/newsreleases/newsrelease-detail/dart-begins-sharing-gtfs-realtime-feed-to-google-and-transit-apps
 *
 * ⚠️ NOTE: DART GTFS-RT feeds may require registration or use of specific URLs.
 * Check https://www.dart.org/riding-dart/schedules/developer-resources for current URLs.
 *
 * Run with: node scripts/collectDataDallas.js
 */
...
const VEHICLE_POSITIONS_URL = process.env.VEHICLE_POSITIONS_URL || "";

// Light Rail route IDs
// Red Line, Blue Line, Green Line, Orange Line
const LIGHT_RAIL_ROUTES = ["RED", "BLUE", "GREEN", "ORANGE"];
...
function calculateSpeed(vehicleId, lat, lon, timestamp) {
  ...
  // Sanity check: DART light rail max speed is around 65 mph
  if (speedMph > 75) {
...
process.on("SIGINT", () => {
  console.log("\n\n👋 Shutting down Dallas collector...");
  process.exit(0);
});
