...
`;
  ...
  try {
    ...
    console.log(
      `   Found ${data.elements.length} total grade crossings in bounding box`,
    );
    ...
    console.log(
      `   ✂️  Filtered to ${filteredFeatures.length} crossings near transit lines`,
    );

    // Cluster nearby crossings to reduce visual clutter
    const clusteredFeatures = clusterCrossings(filteredFeatures);
    console.log(
      `   🔗 Clustered to ${clusteredFeatures.length} markers (merged crossings within ${CLUSTER_METERS}m)`,
    );
    ...
    if (cityKey === "SF") {
      ...
      finalFeatures = clusteredFeatures.filter(
        (f) => !crossingsToDelete.has(f.properties.id),
      );
      if (finalFeatures.length < beforeCount) {
        console.log(
          `   🗑️  Deleted ${beforeCount - finalFeatures.length} manually specified crossings`,
        );
      }
    }
...
async function main() {
  ...
  console.log(
    `   Filtering to crossings within ${PROXIMITY_METERS}m of transit lines`,
  );
  console.log(
    `   Clustering crossings within ${CLUSTER_METERS}m into single markers`,
  );
