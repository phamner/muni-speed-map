...
`;
  ...
  try {
    ...
    console.log(`   Found ${data.elements.length} total ${type}s in bounding box`);
    ...
    console.log(`   Filtered to ${nearbyElements.length} ${type}s near transit lines`);
...
async function processCity(cityKey) {
  ...
  console.log(`   ✅ Saved ${switches.features.length} switches to ${city.switchesFile}`);
  ...
  console.log(`   ✅ Saved ${signals.features.length} signals to ${city.signalsFile}`);
}
