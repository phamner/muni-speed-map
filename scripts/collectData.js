  ...
  try {
    ...
    const metroVehicles = vehicleActivities
      ...
      .map((v) => {
        ...
        const headsign =
          journey?.MonitoredCall?.DestinationDisplay ||
        ...
        return {
          ...
          speed_reported: journey?.Velocity
            ? parseFloat(journey.Velocity)
            : null,
...
async function runCollector() {
  ...
  console.log(
    `   Polling every ${POLL_INTERVAL_MS / 1000} seconds (within 511 API rate limit)`,
  );
