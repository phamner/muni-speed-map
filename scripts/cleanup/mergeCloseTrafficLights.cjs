...
function mergeCity(cityKey, dryRun) {
  ...
  console.log(`   Found ${multiClusters.length} clusters to merge (${multiClusters.reduce((s, c) => s + c.length, 0)} lights → ${multiClusters.length} merged)`);
  ...
  if (dryRun) {
    ...
    for (const cluster of multiClusters) {
      ...
      const routes = [...new Set(items.flatMap((f) => f.properties.routes || []))];
      ...
      console.log(`   • ${cluster.length} lights [${ids}] routes=[${routes}] dist=${distances.join("/")}m`);
  ...
  const merged = clusters.map((cluster) => {
    ...
    const crossingIds = [
      ...new Set(
        items
          .map((f) => f.properties.crossing_id)
          .filter((id) => id != null),
      ),
    ];
  ...
  console.log(`   ✅ Merged: ${features.length} → ${merged.length} (removed ${removedCount} duplicates)`);
}
