...
const OUTPUT_FILE = path.join(
  ...
  "data",
  "rail-context",
  "torontoFerryRoutesOverlay.json",
);
...
const BBOX = [43.45, -79.75, 44.2, -78.8];
...
async function fetchOverpass(query) {
  ...
  for (const endpoint of OVERPASS_ENDPOINTS) {
    for (let attempt = 1; attempt <= 2; attempt += 1) {
      try {
        ...
        lastError = error;
        console.warn(
          `Overpass Toronto ferry fetch failed via ${endpoint} (attempt ${attempt}/2): ${error.message}`,
        );
  ...
  throw lastError || new Error("Overpass Toronto ferry fetch failed");
}
...
function normalizeOperator(value) {
  if (!value) return "";
  if (value.includes("City of Toronto")) return "Toronto Island Ferries";
  if (value.includes("Toronto Island Ferries")) return "Toronto Island Ferries";
  if (value.includes("Billy Bishop Airport")) return "Billy Bishop Airport Ferry";
  return value;
}

function normalizeTerminalName(value) {
  if (!value) return "";
  if (value === "Toronto") return "Jack Layton Ferry Terminal";
  return value.trim();
}
...
const ALLOWED_TORONTO_FERRY_AGENCIES = new Set([
...
function shouldIncludeRelation(relation) {
  ...
  return ALLOWED_TORONTO_FERRY_AGENCIES.has(agency);
}
...
function buildFeature(relation) {
  ...
  const routeName = normalizeTerminalName(
    String(
      relation.tags?.name || relation.tags?.network || agencyName || "Ferry route",
    ).trim(),
  );
  const fromTerminal = normalizeTerminalName(
    String(relation.tags?.from || "").trim(),
  );
  ...
  return {
    ...
    properties: {
      ...
      route_short_name: buildRouteShortName({
        ...relation.tags,
        from: fromTerminal,
        to: toTerminal,
      }),
    ...
    merged.push({
      ...
      properties: {
        ...
        route_name:
          allTerminals.length === 2
            ? buildBidirectionalLabel(allTerminals[0], allTerminals[1])
            : String(props.route_name || "Ferry route").trim(),
...
`;
  ...
  console.log("Fetching Toronto ferry routes from OSM...");
  ...
  console.log(`Wrote ${features.length} Toronto ferry routes to ${OUTPUT_FILE}`);
}
