  ...
  try {
    ...
    const features = data.elements
      ...
      .map(element => {
        ...
        const color = city.lineColors[route_id] || city.lineColors['default'] || '#666666';
    ...
    console.error(`   ❌ Error fetching routes for ${city.name}:`, error.message);
    return 0;
  }
}
...
`;
  ...
  try {
    ...
    fs.writeFileSync(outputPath, JSON.stringify(geojson, null, 2));
    console.log(`   ✅ Saved ${features.length} stops to ${city.outputStopsFile}`);
    
    return features.length;
  } catch (error) {
    console.error(`   ❌ Error fetching stops for ${city.name}:`, error.message);
    return 0;
  }
}
...
async function main() {
  ...
  for (const [city, counts] of Object.entries(results)) {
    console.log(`  ${CITIES[city].name}: ${counts.routes} route segments, ${counts.stops} stops`);
  }
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
