...
// Bounding box around the B Line corridor (Union Station to North Hollywood).
// Fetch the B Line relation and resolve its member ways.
const QUERY = `[out:json][timeout:120];
(
  way["railway"="subway"]["network"~"LACMTA|Los Angeles Metro|Metro"]["name"~"Metro B Line|Red Line",i](33.95,-118.45,34.20,-118.20);
  relation["type"="route"]["route"="subway"]["ref"="B"](33.95,-118.45,34.20,-118.20);
  way(r);
);
out body;
>;
...
function isBLineWay(tags) {
  const name = String(tags?.name || "").toLowerCase();
  return (
    name.includes("metro b line") ||
    name.includes("red line") ||
    String(tags?.ref || "") === "B"
  );
}

/** Collect way IDs that are members of any B Line relation. */
...
async function main() {
  console.log("Fetching full LA B Line from OSM/Overpass...");
  ...
  const features = ways
    .filter((way) => isBLineWay(way.tags || {}) || relationMemberIds.has(way.id))
  ...
  console.log(`Wrote ${deduped.length} B Line features:`);
  console.log(
